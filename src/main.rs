...
struct Streams {
    prev_token: Token,
    streams: BTreeMap<Token,(TcpStream,TcpConnection)>, // TODO use slab probably
}
impl Streams {
    fn new(min_token: Token) -> Self {
        Streams { prev_token: min_token,  streams: BTreeMap::new() }
    }
    fn add(&mut self,  source: &TcpListener,  typ: TcpConnection,  look_for: Ready, poll: &Poll) {
        loop {
            match source.accept() {
                ...
                Ok((stream, _addr)) => {
                    self.prev_token.0 += 1;
                    if self.prev_token.0 == usize::MAX {
                        panic!("Too many streams have been opened");
                        // TODO reregister all
                    }
                    let token = self.prev_token;
                    match poll.register(&stream, token, look_for, PollOpt::edge()) {
                        Err(e) => eprintln!("Cannot register TCP connection: {}", e),
                        Ok(()) => {self.streams.insert(token, (stream, typ));}
                    }
...
fn main() {
    ...
    loop {
        ...
        println!("connections: {}, events: {}, last token: {}", streams.streams.len(), events.len(), streams.prev_token.0);
        for event in events.iter() {
            ...
            match event.token() {
                ...
                conn_token => {
                    let mut oe = match streams.streams.entry(conn_token) {
