...
in_port_t getport(int sock, int(*getside)(int, struct sockaddr*, socklen_t*)) {
    struct sockaddr_storage addr;
    socklen_t len = sizeof(struct sockaddr_storage);
    checkerr(getside(sock, (struct sockaddr*)&addr, &len), "get address for socket");
    switch (addr.ss_family) {
        ...
        default:
            fprintf(stderr, "Cannot get port from address of unexpected type %d\n", addr.ss_family);
            return 0;
    }
}
...
int connect_to_localhost(const char *port) {
    ...
    int sock = checkerr(socket(AF_INET, SOCK_DGRAM, IPPROTO_UDPLITE), "create UDPlite socket");
...
// * Fall back to UDP if creating the socket fails or no response is received (if expected).
